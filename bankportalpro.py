...
def deposit_db(accid, amount):
    ...
    row = cur.fetchone()
    if not row: con.close(); return False, "Account not found"
    if row[1].lower() == "closed": con.close(); return False, "Account closed"
...
def withdraw_db(accid, amount):
    ...
    if not row: con.close(); return False, "Account not found"
    if row[1].lower() == "closed": con.close(); return False, "Account closed"
    if Decimal(row[0]) < Decimal(amount): con.close(); return False, "Insufficient funds"
    new_bal = Decimal(row[0]) - Decimal(amount)
    cur.execute("UPDATE accounts SET balance=%s WHERE accid=%s", (new_bal, accid))
    cur.execute("INSERT INTO transactions (accid, type, amount, balance_after, note) VALUES (%s,%s,%s,%s,%s)",
...
def transfer_db(src, dst, amount):
    ...
    t = cur.fetchone()
    if not s or not t: con.close(); return False, "Source or destination not found"
    if s[1].lower()=="closed" or t[1].lower()=="closed": con.close(); return False, "One account closed"
    if Decimal(s[0]) < Decimal(amount): con.close(); return False, "Insufficient funds"
    ...
    def open_create(self):
        ...
        def go():
            try:
                ...
                messagebox.showinfo("Success","Account created"); w.destroy(); self.load_accounts()
